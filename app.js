  ...
  const hero = `
    <section class="screen hero active">
      ...
      <div class="hero-content">
        ...
        <p>${h.portada.texto}</p>
        <div class="cta-stack">
          <button class="primary" data-next data-start-music>Empezar nuestra historia <span>→</span></button>
    ...
    <section class="screen family-screen">
      <div class="family-wrap">
        ...
        <div class="constellation">
          ${h.familia.integrantes.map((x,i)=>`
            <div class="person-card">${familyIcon(x)}<b>${x.nombre}</b><small>${x.detalle}</small></div>
            ${i < h.familia.integrantes.length-1 ? '<div class="plus">+</div>' : ''}`).join('')}
        </div>
  ...
  const gallery = `
    <section class="screen gallery-screen">
      <div class="gallery-copy">
        <span class="eyebrow">NUESTROS RECUERDOS</span>
        <h2>Un montón de fotos. Seis años. Una familia.</h2>
        <p>Quise guardar aquí muchos pedacitos de nosotros: tú, yo, George, Mía y todas esas pequeñas aventuras que hicieron hogar.</p>
      </div>
      <div class="collage-wall">
        ${h.collages.map((src,i)=>`<figure class="collage-card"><img data-photo="${src}" alt="Collage de recuerdos ${i+1}"><figcaption>Recuerdos ${i+1}</figcaption></figure>`).join('')}
      </div>
      <div class="highlight-grid">
        ${h.destacados.map((src,i)=>`<img data-photo="${src}" alt="Recuerdo destacado ${i+1}">`).join('')}
      </div>
