...
function render() {
  ...
  const gallery = `<section class="screen gallery-screen"><div class="gallery-copy"><span class="eyebrow">NUESTROS RECUERDOS</span><h2>Fotos, aventuras y nuestra familia.</h2><p>Antes que mostrarte imágenes dañadas, preferí dejar esta pared limpia con recuerdos reales de nosotros, George y Mía. Cada foto aquí sí carga completa.</p></div><div class="memory-mosaic">${mosaicPhotos.map((src,i)=>`<figure class="memory-tile tile-${(i%6)+1}"><img data-photo="${src}" alt="Recuerdo ${i+1}"></figure>`).join('')}</div><button class="primary gallery-next" data-next>Hay algo que todavía no te dije <span>→</span></button></section>`;
